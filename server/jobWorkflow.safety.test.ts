...
describe("job workflow safety rules", () => {
  ...
  it("marks a previously audited employer as Skipped-Duplicate and does not prepare outreach", async () => {
    ...
    db.select
      .mockImplementationOnce(() => selectWithLimit([{ id: 1, ownerOpenId: "owner-id", runKey: "run-key" }]))
      .mockImplementationOnce(() => selectWithJoinWhere([{ employer: "Aurobindo Pharma" }]))
    ...
    mocks.storagePut
      .mockResolvedValueOnce({ key: "reports/run-key.md", url: "https://storage.example/report" })
      .mockResolvedValueOnce({ key: "audits/run-key.jsonl", url: "https://storage.example/audit" });
    ...
    expect(leadValues).toHaveBeenCalledWith(expect.objectContaining({ status: "Skipped-Duplicate" }));
    expect(db.insert).toHaveBeenCalledTimes(3);
    expect(result).toMatchObject({ totalAudited: 1, preparedCount: 0, skippedCount: 1 });
    expect(mocks.notifyOwner).toHaveBeenCalledWith(expect.objectContaining({ content: expect.stringContaining("0 new drafts") }));
  });
  ...
  it("records explicit confirmation as held and sends nothing when no delivery provider is configured", async () => {
    ...
    db.select.mockImplementationOnce(() => selectWithJoinWhereLimit([{
      draft: {
        id: 7,
        leadId: 55,
    ...
    expect(result).toEqual({ alreadySent: false, deliveryConfigured: false, deliverySent: false });
    expect(updateSet).toHaveBeenCalledWith(expect.objectContaining({ confirmedAt: expect.any(Date) }));
    expect(eventValues).toHaveBeenCalledWith(expect.objectContaining({ eventType: "confirmation-held" }));
  });
});
