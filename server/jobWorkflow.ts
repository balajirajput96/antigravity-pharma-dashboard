...
function normalizeStatus(status?: string): LeadStatus {
  return status && statusSet.has(status as LeadStatus)
...
function normalizedEmployer(value: string) {
  return value
    .toLowerCase()
    .replace(
      /\b(private|pvt|limited|ltd|llp|inc|incorporated|co|company)\b/g,
      ""
    )
    .replace(/[^a-z0-9]/g, "")
    .slice(0, 80);
...
async function analyzeWithLlm(
  ...
  if (duplicate) {
    return {
      ...
      roleFit:
        "Historical audit contains an employer already contacted or reviewed.",
      eligibilityNotes: "Skipped to prevent duplicate outreach.",
    };
  }
      ...
      json_schema: {
        ...
        schema: {
          ...
          properties: {
            status: {
              type: "string",
              enum: ["Prepared", "Skipped-Role mismatch"],
            },
            roleFit: { type: "string" },
  ...
  if (typeof content !== "string")
    throw new Error("LLM returned an empty job analysis response");
...
function buildReport(
  ...
  const rows = records
    .map(
      (record, index) =>
        `| ${index + 1} | ${record.employer} | ${record.roleTitle} | ${record.status} | ${record.roleFit ?? "—"} | [Source](${record.sourceUrl}) |`
    )
    .join("\n");
    ...
    records
      .map(record =>
        JSON.stringify({
          runDate,
          ...record,
          generatedBy: "Antigravity Pharma Workspace",
          outreach: "confirmation-required",
        })
      )
      .join("\n") + (records.length ? "\n" : "")
  );
...
export async function ensureWorkflowSettings(ownerOpenId: string) {
  ...
  const existing = await db
    ...
    .where(eq(workflowSettings.ownerOpenId, ownerOpenId))
    .limit(1);
  if (existing[0]) return existing[0];
  await db.insert(workflowSettings).values({
    ownerOpenId,
    candidateProfile: JSON.stringify(CANDIDATE_PROFILE),
  });
...
export async function listDashboard(ownerOpenId: string) {
  ...
  const runs = await db
    .select()
    .from(jobRuns)
    .where(eq(jobRuns.ownerOpenId, ownerOpenId))
  ...
  const files = await db
    .select()
    .from(workspaceFiles)
    .where(eq(workspaceFiles.ownerOpenId, ownerOpenId))
    .orderBy(desc(workspaceFiles.createdAt))
    .limit(20);
  const drafts = await db
    .select({ draft: outreachDrafts, lead: jobLeads })
    .from(outreachDrafts)
  ...
  const recentLeads = latest
    ? await db
        .select()
        .from(jobLeads)
        .where(eq(jobLeads.runId, latest.id))
        .orderBy(desc(jobLeads.id))
        .limit(12)
    : [];
  return {
    settings,
...
export async function createInstructionFile(ownerOpenId: string) {
  ...
  const stored = await storagePut(
    `${ownerOpenId}/instructions/${filename}`,
  ...
  const [file] = await db
    ...
    .where(
      and(
        eq(workspaceFiles.ownerOpenId, ownerOpenId),
        eq(workspaceFiles.storageKey, stored.key)
      )
    )
    .limit(1);
  ...
  await db
    ...
    .set({ agentInstructionFileId: file.id })
    .where(eq(workflowSettings.ownerOpenId, ownerOpenId));
  return file;
}

export async function ingestJobRun(
  ownerOpenId: string,
  rawLeads: IngestedLead[],
  runMode: "scheduled" | "manual-import" = "scheduled"
) {
  const db = await getDb();
  if (!db) throw new Error("Database is unavailable");
  ...
  const [run] = await db
    ...
    .from(jobRuns)
    .where(
      and(eq(jobRuns.ownerOpenId, ownerOpenId), eq(jobRuns.runKey, runKey))
    )
    .limit(1);
  if (!run) throw new Error("Run was not created");

  const historical = await db
    .select({ employer: jobLeads.employer })
    .from(jobLeads)
    .innerJoin(jobRuns, eq(jobLeads.runId, jobRuns.id))
    .where(eq(jobRuns.ownerOpenId, ownerOpenId));
  const employers = new Set(
    historical.map(item => normalizedEmployer(item.employer))
  );
  ...
  for (const raw of rawLeads.slice(0, 30)) {
    ...
    try {
      analysis = raw.status
        ? {
            ...
            roleFit: raw.roleFit ?? "Imported public-vacancy review.",
            eligibilityNotes: raw.eligibilityNotes ?? "",
            subject: raw.draftSubject,
            body: raw.draftBody,
          }
        : await analyzeWithLlm(raw, duplicate);
    } catch (error) {
      analysis = {
        status: "Skipped-Role mismatch",
        roleFit: "Analysis could not safely verify fit.",
        eligibilityNotes: String(error),
      };
    }
    ...
    if (
      ...
      await db.insert(outreachDrafts).values({
        runId: run.id,
        leadId: lead.id,
        recipientEmail: raw.publicContactEmail,
  ...
  const reportStored = await storagePut(
    `${ownerOpenId}/reports/${runKey}.md`,
    report,
    "text/markdown"
  );
  const auditStored = await storagePut(
    `${ownerOpenId}/audits/${runKey}.jsonl`,
    audit,
  ...
  await db.insert(workspaceFiles).values([
    {
      ...
      runId: run.id,
      kind: "report",
      filename: `pharma-job-report-${today}.md`,
      storageKey: reportStored.key,
      storageUrl: reportStored.url,
      mimeType: "text/markdown",
    },
    {
      ownerOpenId,
      runId: run.id,
      kind: "audit",
      filename: `pharma-job-audit-${today}.jsonl`,
      storageKey: auditStored.key,
      storageUrl: auditStored.url,
  ...
  const linkedFiles = await db
    .select()
    .from(workspaceFiles)
    .where(eq(workspaceFiles.runId, run.id));
  const reportFile = linkedFiles.find(file => file.kind === "report");
  const auditFile = linkedFiles.find(file => file.kind === "audit");
  const preparedCount = createdLeads.filter(
    lead => lead.status === "Prepared"
  ).length;
  const sentCount = createdLeads.filter(
    lead => lead.status === "Verified-Sent"
  ).length;
  ...
  await db
    ...
    .set({
      status: "completed",
      totalAudited: createdLeads.length,
      preparedCount,
      sentCount,
      skippedCount,
      reportFileId: reportFile?.id ?? null,
      auditFileId: auditFile?.id ?? null,
      completedAt: new Date(),
    })
    .where(eq(jobRuns.id, run.id));
  await notifyOwner({
    title: "Daily pharma search complete",
    content: `${preparedCount} new drafts are ready for Balaji Rajput. ${skippedCount} leads were skipped. Review is required before any outreach.`,
  });
  return {
    runId: run.id,
    preparedCount,
...
export async function recordConfirmedSend(
  ownerOpenId: string,
  draftId: number
) {
  const db = await getDb();
  if (!db) throw new Error("Database is unavailable");
  const rows = await db
    .select({ draft: outreachDrafts, run: jobRuns })
    .from(outreachDrafts)
    .innerJoin(jobRuns, eq(outreachDrafts.runId, jobRuns.id))
    .where(
      and(eq(outreachDrafts.id, draftId), eq(jobRuns.ownerOpenId, ownerOpenId))
    )
    .limit(1);
  const row = rows[0];
  ...
  if (!webhookUrl) {
    ...
    await db.insert(deliveryEvents).values({
      ...
      details:
        "Owner pressed Confirm & Send, but no outbound provider is configured. Nothing was delivered.",
    });
    return {
      alreadySent: false,
  ...
  if (!deliveryResponse.ok) {
    ...
    throw new Error(
      `Configured delivery provider rejected this confirmed send (${deliveryResponse.status})${detail ? `: ${detail}` : ""}`
    );
  }
  const deliveryPayload = (await deliveryResponse.json().catch(() => ({}))) as {
    id?: string;
    messageId?: string;
  };
  const deliveryReference =
    deliveryPayload.messageId ??
    deliveryPayload.id ??
    `confirmed-${draftId}-${Date.now()}`;
  ...
  await db
    .update(jobLeads)
    .set({ status: "Verified-Sent" })
    .where(eq(jobLeads.id, row.draft.leadId));
  await db
    .update(jobRuns)
    .set({
