...
|---|---|---|
| Google Gemini web app | Authenticated browser session observed as `balajirajput966@gmail.com`. The Gemini app rendered its signed-in interface and Google Account control. | Browser session only; no credentials, tokens, or account-private content were accessed. This is a different account from the owner account used through the Antigravity entry path. |
