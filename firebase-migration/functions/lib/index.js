...
initializeApp();
export const ownerBootstrap = onCall(
  { secrets: [ownerEmailSecret] },
  async request => {
    if (!request.auth)
 ...
 */
export const recordConfirmationHold = onCall(
  { secrets: [ownerEmailSecret] },
  async request => {
    if (!request.auth)
      throw new HttpsError("unauthenticated", "Google sign-in is required.");
    ...
    await db.runTransaction(async transaction => {
      ...
      if (!draft.exists) throw new HttpsError("not-found", "Draft not found.");
      const status = String(draft.data()?.status ?? "");
      if (!canRecordConfirmationHold(status, false)) {
        throw new HttpsError(
          "failed-precondition",
          "Only a Prepared draft can be confirmed while delivery is disabled."
        );
      }
      transaction.set(eventRef, {
        ...
        details:
          "Owner explicitly confirmed; no delivery provider is configured, so no message was sent.",
 ...
 */
export const validateApprovedImport = onCall(
  { secrets: [ownerEmailSecret] },
  async request => {
    if (!request.auth)
      throw new HttpsError("unauthenticated", "Google sign-in is required.");
    ...
    if (!isApprovedImport(kind, mimeType)) {
      throw new HttpsError(
        "invalid-argument",
        "Only approved Hindi reports and JSONL audit files may be imported."
      );
    }
    return {
      accepted: true,
      uploadPathTemplate: "owners/{ownerUid}/workspace/{fileId}",
    };
  }
);
