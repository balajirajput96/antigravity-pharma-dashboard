...
describe("reels master indexes", () => {
  it("keeps verified counts, Batch_001 records, and safety boundaries consistent", () => {
    const state = readJson<MasterState>(
    ...
    const assets = readJson<{
      entries: {
        ...
        narrationTechnicalIntegrity?: string;
      }[];
    ...
    const errors = readJson<{
      reelFailureCount: number;
      retryQueue: unknown[];
    ...
    expect(research.records.map(({ reelId }) => reelId)).toEqual([
      "0001",
      "0002",
      "0003",
      "0004",
    ...
    expect(
      research.records
        .filter(({ usedStatus }) => usedStatus === "used")
        .map(({ reelId }) => reelId)
    ...
    expect(
      research.records.find(({ reelId }) => reelId === "0004")
    ).toMatchObject({
      usedStatus: "not-yet-produced",
      reelPotential: "evidence-verified-not-yet-scripted",
    });
    expect(assets.entries.map(({ reelId }) => reelId)).toEqual([
      "0001",
      "0002",
      "0003",
    ]);
    expect(quality.entries.map(({ reelId }) => reelId)).toEqual([
      "0001",
      "0002",
      "0003",
    ]);
    expect(
      assets.entries
        .filter(({ packageStatus }) =>
          packageStatus.startsWith("private-drive-verified")
        )
        .map(({ reelId }) => reelId)
    ).toEqual(["0001", "0002", "0003"]);
    expect(
      assets.entries.find(({ reelId }) => reelId === "0003")
    ).toMatchObject({
      packageStatus: "private-drive-verified-review-ready",
    });
    expect(
      assets.entries.find(({ reelId }) => reelId === "0003")
        ?.narrationTechnicalIntegrity
    ).toContain("47.440 seconds");
    expect(
      quality.entries.every(({ status }) =>
        status.startsWith("passed-private-drive-verified")
      )
    ).toBe(true);
    expect(errors.reelFailureCount).toBe(state.masterProgress.failed.count);
    expect(errors.retryQueue).toHaveLength(
      state.masterProgress.retryQueue.length
    );
    ...
    expect(state.masterProgress.lastInventory).toMatchObject({
      ...
      reconciliationRecord:
        "automation/reels/DRIVE_INVENTORY_RECONCILIATION_2026-08-23.md",
      candidateFolderInventory:
        "automation/reels/drive-candidate-folder-inventory.json",
      candidateFolderCounts: {
        totalFolders: 94,
    ...
    expect(
      state.masterProgress.lastInventory.unclassifiedArtifactsRule
    ).toContain("independently validated");
    ...
    expect(
      candidateInventory.candidateAudits[0].reelIdentifierCollision
    ).toContain("must not be merged");
    expect(
      candidateInventory.candidateAudits[0].releaseHoldReasons
    ).toHaveLength(4);
