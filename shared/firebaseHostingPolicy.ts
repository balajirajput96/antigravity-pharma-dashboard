...
export function mayRecordConfirmationHold(status: string, deliveryConfigured = false) {
...
export function isApprovedWorkspaceImport(kind: string, mimeType: string) {
  return (kind === "hindi-report" && mimeType === "text/markdown") || (kind === "jsonl-audit" && mimeType === "application/x-ndjson");
}
