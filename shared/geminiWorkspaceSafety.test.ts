...
const templateDir = resolve(
  process.cwd(),
  "automation/gemini-workspace-template"
);

describe("Gemini workspace recovery template", () => {
  it("preserves the required research-only safety gates", () => {
    const instructions = readFileSync(
      resolve(templateDir, "GEMINI.md"),
      "utf8"
    );
    const policy = readFileSync(
      resolve(templateDir, "gemini_pharma_safety.toml"),
      "utf8"
    );
    const pnpmWorkspace = readFileSync(
      resolve(templateDir, "pnpm-workspace.yaml"),
      "utf8"
    );
    ...
    expect(instructions).toContain(
      "existing Gemini authentication is unavailable"
    );
    expect(instructions).toContain(
      "Do not open, start, or guide any login flow"
    );
    ...
    expect(pnpmWorkspace).toMatch(/['"]?@github\/keytar['"]?: false/);
  ...
  it("contains only non-secret recovery source files", () => {
    ...
    const launcher = readFileSync(
      resolve(templateDir, "gemini_pharma"),
      "utf8"
    );
