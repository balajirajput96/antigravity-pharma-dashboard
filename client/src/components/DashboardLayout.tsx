...
export default function DashboardLayout({
    ...
    return (
      <main className="grid min-h-screen place-items-center bg-[#07120f] px-5 text-white">
        <section className="w-full max-w-md rounded-[2rem] border border-white/10 bg-[#10221c] p-9 shadow-2xl shadow-black/30">
          <div className="mb-8 flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-400 text-[#07120f]">
          ...
          <p className="mt-4 leading-7 text-white/65">
            This workspace is reserved for Balaji Rajput. Sign in to access
            protected job-search reports, drafts, and safety controls.
          </p>
          <Button
            onClick={() => startLogin()}
            className="mt-8 h-11 w-full bg-emerald-400 font-semibold text-[#07120f] hover:bg-emerald-300"
          >
            Sign in securely
          </Button>
        </section>
  ...
  return (
    ...
    >
      <DashboardShell setSidebarWidth={setSidebarWidth}>
        {children}
      </DashboardShell>
    </SidebarProvider>
  );
  ...
  return (
    <>
      <div className="relative" ref={sidebarRef}>
        <Sidebar
          collapsible="icon"
          className="border-r border-white/5 bg-[#081812] text-white"
        >
          <SidebarHeader className="h-[92px] justify-center border-b border-white/5 px-3">
            <div className="flex items-center gap-3">
              <button
                ...
                className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-white/5 text-emerald-300 transition hover:bg-white/10"
              >
                <PanelLeft size={17} />
              </button>
              {state !== "collapsed" && (
          ...
          <SidebarContent className="px-2 py-5">
              ...
              {menuItems.map(item => (
                <SidebarMenuItem key={item.target}>
                  <SidebarMenuButton
                    ...
                    tooltip={item.label}
                    className="h-11 rounded-xl text-white/65 hover:bg-white/8 hover:text-white"
                  >
                    <item.icon size={17} />
            ...
            {state !== "collapsed" && (
              <div className="mx-2 mt-auto rounded-2xl border border-emerald-300/10 bg-emerald-300/5 p-4">
                <div className="flex items-center gap-2 text-xs font-semibold text-emerald-200">
                  <CalendarClock size={14} /> Daily schedule
                </div>
                <p className="mt-2 text-xs leading-5 text-white/55">
                  10:00 AM IST
                  <br />
                  Review required before outreach.
                </p>
              </div>
            )}
          </SidebarContent>
          <SidebarFooter className="border-t border-white/5 p-3">
            <div className="flex items-center gap-3 rounded-xl px-1 py-2 group-data-[collapsible=icon]:justify-center">
              <Avatar className="h-9 w-9 border border-emerald-300/20">
                <AvatarFallback className="bg-emerald-300 text-xs font-bold text-[#07120f]">
        ...
        {state !== "collapsed" && (
          <button
            ...
            className="absolute right-0 top-0 z-50 h-full w-1 cursor-col-resize hover:bg-emerald-300/30"
      ...
      <SidebarInset className="bg-[#f6f8f5]">
        {isMobile && (
          <header className="sticky top-0 z-30 flex h-16 items-center border-b border-black/5 bg-[#f6f8f5]/90 px-4 backdrop-blur">
